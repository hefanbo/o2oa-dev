...
public class EmailNotificationConsumeQueue extends AbstractQueue<Message> {

    private static Logger logger = LoggerFactory.getLogger(EmailNotificationConsumeQueue.class);
    ...
    private void sendMail(String receiver, String subject) throws MessagingException, UnsupportedEncodingException, Exception {
        ...
        String timeout = Config.emailNotification().getTimeout();
        ...
        logger.info("Sending email notification to {}.", receiver);
        try {
            Transport.send(message);
        } catch (MessagingException e) {
            logger.warn("Failed to send Email notification to {}.", receiver);
            logger.warn(e.toString());
        }
        logger.info("Email notification is sent to {}.", receiver);
    }
}
